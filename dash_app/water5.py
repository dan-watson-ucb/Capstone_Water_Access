...
query = "SELECT country_name, district, sub_district, status_id, fuzzy_water_source, fuzzy_water_tech, management, today_preds_text, one_year_preds_text, one_km_population, impact_text, lat_deg, lon_deg from final_all WHERE country_name = 'Kenya'"
...
def run_query(n_clicks, country, status, district, sub_district, fuzzy_water_source, fuzzy_water_tech, management, today_preds_text, one_year_preds_text, impact_text, point_color_select):
    conn = psycopg2.connect("dbname='water_db' user='dan' host='postgres-instance2.clhlqrsuvowr.us-east-1.rds.amazonaws.com' password='berkeley'")
    base_query = "SELECT lat_deg, lon_deg, status_id, today_preds_text, one_year_preds_text, current_status_text from final_all WHERE country_name =" + "'" + str(country) + "'"
...
## Reset Button
########################################################################################################################################################################
# @app.callback(Output('country-select', 'value'), [Input('reset-button', 'n_clicks')])
...
def run_query(n_clicks, country, status, district, sub_district, fuzzy_water_source, fuzzy_water_tech, management, today_preds_text, one_year_preds_text, impact_text):
    conn = psycopg2.connect("dbname='water_db' user='dan' host='postgres-instance2.clhlqrsuvowr.us-east-1.rds.amazonaws.com' password='berkeley'")
    base_query = "SELECT country_name, district, sub_district, status_id, fuzzy_water_source, fuzzy_water_tech, management,  today_preds_text, one_year_preds_text, one_km_population, one_km_functioning_water_points, impact_text, lat_deg, lon_deg from final_all WHERE country_name =" + "'" + str(country) + "'"
